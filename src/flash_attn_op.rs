...
impl candle_core::CustomOp3 for FlashAttnOp {
    ...
    fn cuda_fwd(
        ...
        k_st: &candle_core::CudaStorage, kl: &Layout,
        v_st: &candle_core::CudaStorage, vl: &Layout,
    ) -> Result<(candle_core::CudaStorage, Shape)> {
        ...
        let (batch, heads, seq, d_head) = match dims {
            &[b, h, s, d] => (b, h, s, d),
            _ => candle_core::bail!("flash_attn: expected [batch, heads, seq, d_head], got {:?}", dims),
        };
    ...
    fn bwd(
        ...
        let lse_slice = self.lse_cache.lock().unwrap().take()
            .ok_or_else(|| candle_core::Error::msg("flash_attn bwd: lse not in cache — was forward called first?"))?;
