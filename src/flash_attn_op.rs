...
extern "C" {
    fn flash_attn_fwd(
        ...
        batch: i32, seq: i32, heads: i32, d_head: i32,
        scale: f32, causal: bool,
    );

    fn flash_attn_bwd(
        q: *const f32, k: *const f32, v: *const f32,
        o: *const f32, do_: *const f32,
...
impl candle_core::CustomOp3 for FlashAttnOp {
    fn name(&self) -> &'static str { "flash_attn" }
    ...
    fn cuda_fwd(
        &self,
        q_st: &candle_core::CudaStorage, ql: &Layout,
        k_st: &candle_core::CudaStorage, _:  &Layout,
        v_st: &candle_core::CudaStorage, _:  &Layout,
    ) -> Result<(candle_core::CudaStorage, Shape)> {
        use candle_core::cuda_backend::{CudaStorageSlice, WrapErr};
        use candle_core::cuda_backend::cudarc::driver::DevicePtr;

        let dims = ql.shape().dims();
        let (batch, seq, heads, d_head) = match dims {
            &[b, s, h, d] => (b, s, h, d),
            _ => candle_core::bail!("flash_attn: expected [batch, seq, heads, d_head], got {:?}", dims),
        };
        let n     = batch * seq * heads * d_head;
        ...
        unsafe {
            flash_attn_fwd(
                ...
                batch as i32, seq as i32, heads as i32, d_head as i32,
                self.scale, self.causal,
            );
        }
        // lse is dropped — backward recomputes it from Q and K.
    ...
    fn bwd(
        ...
        let (batch, seq, heads, d_head) = q.dims4()?;
        let n = batch * seq * heads * d_head;
        ...
        // Allocate gradient buffers (kernel will zero dk and dv internally)
...
/// q, k, v: [batch, seq, heads, d_head] f32 on CUDA.
/// Returns:  [batch, seq, heads, d_head] f32.
pub fn flash_attn(
    q: &Tensor, k: &Tensor, v: &Tensor,
    scale: f32, causal: bool,
) -> Result<Tensor> {
    q.apply_op3(k, v, FlashAttnOp { scale, causal })
}
